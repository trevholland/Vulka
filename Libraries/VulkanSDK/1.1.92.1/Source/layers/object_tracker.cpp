...
void ReportUndestroyedObjects(VkDevice device, const std::string& error_code) {
    ...
    DeviceReportUndestroyedObjects(device, kVulkanObjectTypeAccelerationStructureNV, error_code);
}
    ...
    VkAccelerationStructureNV*                  pAccelerationStructure)
{
    ...
    if (skip) return VK_ERROR_VALIDATION_FAILED_EXT;
    VkResult result = GetLayerDataPtr(get_dispatch_key(device), layer_data_map)->device_dispatch_table.CreateAccelerationStructureNV(device, pCreateInfo, pAllocator, pAccelerationStructure);
    if (VK_SUCCESS == result) {
        std::lock_guard<std::mutex> lock(global_lock);
    ...
    const VkAllocationCallbacks*                pAllocator)
{
    ...
    {
        std::lock_guard<std::mutex> lock(global_lock);
        DestroyObject(device, accelerationStructure, kVulkanObjectTypeAccelerationStructureNV, pAllocator, kVUIDUndefined, kVUIDUndefined);
    }
    if (skip) return;
    GetLayerDataPtr(get_dispatch_key(device), layer_data_map)->device_dispatch_table.DestroyAccelerationStructureNV(device, accelerationStructure, pAllocator);
    ...
    VkMemoryRequirements2KHR*                   pMemoryRequirements)
{
    ...
    if (skip) return;
    GetLayerDataPtr(get_dispatch_key(device), layer_data_map)->device_dispatch_table.GetAccelerationStructureMemoryRequirementsNV(device, pInfo, pMemoryRequirements);
    ...
    const VkBindAccelerationStructureMemoryInfoNV* pBindInfos)
{
    ...
    if (skip) return VK_ERROR_VALIDATION_FAILED_EXT;
    VkResult result = GetLayerDataPtr(get_dispatch_key(device), layer_data_map)->device_dispatch_table.BindAccelerationStructureMemoryNV(device, bindInfoCount, pBindInfos);
    ...
    VkDeviceSize                                scratchOffset)
{
    ...
    if (skip) return;
    GetLayerDataPtr(get_dispatch_key(commandBuffer), layer_data_map)->device_dispatch_table.CmdBuildAccelerationStructureNV(commandBuffer, pInfo, instanceData, instanceOffset, update, dst, src, scratch, scratchOffset);
    ...
    VkCopyAccelerationStructureModeNV           mode)
{
    ...
    if (skip) return;
    GetLayerDataPtr(get_dispatch_key(commandBuffer), layer_data_map)->device_dispatch_table.CmdCopyAccelerationStructureNV(commandBuffer, dst, src, mode);
    ...
    uint32_t                                    depth)
{
    ...
    if (skip) return;
    GetLayerDataPtr(get_dispatch_key(commandBuffer), layer_data_map)->device_dispatch_table.CmdTraceRaysNV(commandBuffer, raygenShaderBindingTableBuffer, raygenShaderBindingOffset, missShaderBindingTableBuffer, missShaderBindingOffset, missShaderBindingStride, hitShaderBindingTableBuffer, hitShaderBindingOffset, hitShaderBindingStride, callableShaderBindingTableBuffer, callableShaderBindingOffset, callableShaderBindingStride, width, height, depth);
    ...
    VkPipeline*                                 pPipelines)
{
    ...
    if (skip) return VK_ERROR_VALIDATION_FAILED_EXT;
    VkResult result = GetLayerDataPtr(get_dispatch_key(device), layer_data_map)->device_dispatch_table.CreateRayTracingPipelinesNV(device, pipelineCache, createInfoCount, pCreateInfos, pAllocator, pPipelines);
    if (VK_SUCCESS == result) {
        std::lock_guard<std::mutex> lock(global_lock);
        for (uint32_t index = 0; index < createInfoCount; index++) {
            CreateObject(device, pPipelines[index], kVulkanObjectTypePipeline, pAllocator);
    ...
    void*                                       pData)
{
    ...
    if (skip) return VK_ERROR_VALIDATION_FAILED_EXT;
    VkResult result = GetLayerDataPtr(get_dispatch_key(device), layer_data_map)->device_dispatch_table.GetRayTracingShaderGroupHandlesNV(device, pipeline, firstGroup, groupCount, dataSize, pData);
    ...
    void*                                       pData)
{
    ...
    if (skip) return VK_ERROR_VALIDATION_FAILED_EXT;
    VkResult result = GetLayerDataPtr(get_dispatch_key(device), layer_data_map)->device_dispatch_table.GetAccelerationStructureHandleNV(device, accelerationStructure, dataSize, pData);
    ...
    uint32_t                                    firstQuery)
{
    ...
    if (skip) return;
    GetLayerDataPtr(get_dispatch_key(commandBuffer), layer_data_map)->device_dispatch_table.CmdWriteAccelerationStructuresPropertiesNV(commandBuffer, accelerationStructureCount, pAccelerationStructures, queryType, queryPool, firstQuery);
    ...
    uint32_t                                    shader)
{
    ...
    if (skip) return VK_ERROR_VALIDATION_FAILED_EXT;
    VkResult result = GetLayerDataPtr(get_dispatch_key(device), layer_data_map)->device_dispatch_table.CompileDeferredNV(device, pipeline, shader);
