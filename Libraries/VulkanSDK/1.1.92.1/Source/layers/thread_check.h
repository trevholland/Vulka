    ...
    VkAccelerationStructureNV*                  pAccelerationStructure)
{
    ...
    result = pTable->CreateAccelerationStructureNV(device,pCreateInfo,pAllocator,pAccelerationStructure);
    if (threadChecks) {
        finishReadObject(my_data, device);
    } else {
        finishMultiThread();
    ...
    const VkAllocationCallbacks*                pAllocator)
{
    ...
    pTable->DestroyAccelerationStructureNV(device,accelerationStructure,pAllocator);
    if (threadChecks) {
        finishReadObject(my_data, device);
    ...
    VkMemoryRequirements2KHR*                   pMemoryRequirements)
{
    ...
    pTable->GetAccelerationStructureMemoryRequirementsNV(device,pInfo,pMemoryRequirements);
    if (threadChecks) {
        finishReadObject(my_data, device);
    } else {
        finishMultiThread();
    ...
    const VkBindAccelerationStructureMemoryInfoNV* pBindInfos)
{
    ...
    result = pTable->BindAccelerationStructureMemoryNV(device,bindInfoCount,pBindInfos);
    if (threadChecks) {
        finishReadObject(my_data, device);
    } else {
        finishMultiThread();
    ...
    VkDeviceSize                                scratchOffset)
{
    ...
    pTable->CmdBuildAccelerationStructureNV(commandBuffer,pInfo,instanceData,instanceOffset,update,dst,src,scratch,scratchOffset);
    if (threadChecks) {
        finishReadObject(my_data, commandBuffer);
    ...
    VkCopyAccelerationStructureModeNV           mode)
{
    ...
    pTable->CmdCopyAccelerationStructureNV(commandBuffer,dst,src,mode);
    if (threadChecks) {
        finishReadObject(my_data, commandBuffer);
        finishReadObject(my_data, dst);
        finishReadObject(my_data, src);
    ...
    uint32_t                                    depth)
{
    ...
    if (threadChecks) {
        ...
        startReadObject(my_data, callableShaderBindingTableBuffer);
    }
    pTable->CmdTraceRaysNV(commandBuffer,raygenShaderBindingTableBuffer,raygenShaderBindingOffset,missShaderBindingTableBuffer,missShaderBindingOffset,missShaderBindingStride,hitShaderBindingTableBuffer,hitShaderBindingOffset,hitShaderBindingStride,callableShaderBindingTableBuffer,callableShaderBindingOffset,callableShaderBindingStride,width,height,depth);
    if (threadChecks) {
        finishReadObject(my_data, commandBuffer);
        ...
        finishReadObject(my_data, callableShaderBindingTableBuffer);
    } else {
        finishMultiThread();
    }
}
    ...
    VkPipeline*                                 pPipelines)
{
    ...
    result = pTable->CreateRayTracingPipelinesNV(device,pipelineCache,createInfoCount,pCreateInfos,pAllocator,pPipelines);
    if (threadChecks) {
        finishReadObject(my_data, device);
        finishReadObject(my_data, pipelineCache);
    } else {
    ...
    void*                                       pData)
{
    ...
    result = pTable->GetRayTracingShaderGroupHandlesNV(device,pipeline,firstGroup,groupCount,dataSize,pData);
    if (threadChecks) {
        finishReadObject(my_data, device);
        finishReadObject(my_data, pipeline);
    } else {
    ...
    void*                                       pData)
{
    ...
    result = pTable->GetAccelerationStructureHandleNV(device,accelerationStructure,dataSize,pData);
    if (threadChecks) {
        finishReadObject(my_data, device);
        finishReadObject(my_data, accelerationStructure);
    } else {
    ...
    uint32_t                                    firstQuery)
{
    ...
    if (threadChecks) {
        ...
        for (uint32_t index = 0; index < accelerationStructureCount; index++) {
            startReadObject(my_data, pAccelerationStructures[index]);
        }
        startReadObject(my_data, queryPool);
    }
    pTable->CmdWriteAccelerationStructuresPropertiesNV(commandBuffer,accelerationStructureCount,pAccelerationStructures,queryType,queryPool,firstQuery);
    if (threadChecks) {
        finishReadObject(my_data, commandBuffer);
        for (uint32_t index = 0; index < accelerationStructureCount; index++) {
            finishReadObject(my_data, pAccelerationStructures[index]);
        }
        finishReadObject(my_data, queryPool);
    } else {
        finishMultiThread();
    ...
    uint32_t                                    shader)
{
    ...
    result = pTable->CompileDeferredNV(device,pipeline,shader);
    if (threadChecks) {
        finishReadObject(my_data, device);
        finishReadObject(my_data, pipeline);
    } else {
